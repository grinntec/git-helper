...
class UserChoice(Enum):
    ...
    PUSH = ('2', 'Push changes to remote repository')    
...
def commit_changes(repo):
    # Fetches both the untracked files, the modified ones, and the staged ones.
    ...
    # Display the modified and staged files
    ...
    print(f"{QUESTION_TEXT}Staged files:{RESET_TEXT}")
...
def add_files(repo):
    ...
    # Ask user if they want to add all or specific files
    user_decision = input(f"{QUESTION_TEXT}Would you like to add all files? (yes/no): {RESET_TEXT}")
...
def main():
    ...
    while True:
        ...
        # Convert enum values to just their number (the first item of the tuple) for comparison
