...
def add_files(repo):
    ...
    # If there are no untracked files, notify the user and return
    if not untracked_files:
        logger.info(f"{ANSWER_TEXT}No untracked files found.{RESET_TEXT}")
        return
    ...
    user_decision = input(f"{QUESTION_TEXT}Would you like to add all untracked files? (yes/no): {RESET_TEXT}")
        ...
        if user_decision.lower() == 'yes':
            ...
            for file in files_to_add:
                if file in untracked_files:
                    ...
                    logger.warning(f"{WARNING_TEXT}File {file} is not an untracked file and was skipped.{RESET_TEXT}")
